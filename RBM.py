...
class RBM:
    ...
    def adam(self, g, epoch, index):
        ...
        self.v[index] = self.beta_2 * self.v[index] + (1 - self.beta_2) * torch.pow(
            g, 2
        )

        m_hat = self.m[index] / (1 - np.power(self.beta_1, epoch)) + (
            1 - self.beta_1
        ) * g / (1 - np.power(self.beta_1, epoch))
    ...
    def train(self, dataset):
        ...
        for epoch in learning:
            ...
            for batch_start_index in range(
                0, dataset.shape[0] - self.batch_size, self.batch_size
            ):
            ...
            details = {
                "epoch": epoch + 1,
                "loss": round(train_loss.item() / counter, 4),
            }
            learning.set_description(str(details))
            learning.refresh()

            if (
                train_loss.item() / counter > self.previous_loss_before_stagnation
                and epoch > self.early_stopping_patience + 1
            ):
                self.stagnation += 1
